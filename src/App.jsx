...
// The internal notes array always uses standard notation (with A# for Bb).
const notes = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];

// Helper function to convert standard note names to Polish notation if enabled.
const getDisplayNote = (note, isPolish) => {
  if (!isPolish) return note;
  if (note === 'B') return 'H';
  if (note === 'A#') return 'B'; // A# is enharmonically Bb
  return note;
};

// Component to render the fretboard diagram
const FretboardDiagram = ({ targetNote, usePolishNotation }) => {
  const positions = [];
  ...
  return (
    <div className="mt-6 font-sans">
      <h3 className="text-lg font-bold text-white mb-3 text-center">'{getDisplayNote(targetNote, usePolishNotation)}' Positions</h3>
...
const App = () => {
  ...
  // State for Polish notation toggle
  const [usePolishNotation, setUsePolishNotation] = useState(false);
      ...
      if (rms > volumeThreshold) {
        ...
        if (clarity > 0.9) {
          ...
          if (detectedNote === targetNote) {
            ...
            setFeedback(`Incorrect. You played ${getDisplayNote(detectedNote, usePolishNotation)}. Play low E for a hint or high E (12th fret) for a new note.`);
            setIsCorrect(false);
            setIsNoteHeld(false);
          }
        }
  ...
  }, [isListening, targetNote, isNoteHeld, lastOctave, isPlaybackActive, usePolishNotation]);

  useEffect(() => {
    if (requiredSuccesses > 0 && successCount >= requiredSuccesses) {
      setFeedback(`You found ${getDisplayNote(targetNote, usePolishNotation)} ${requiredSuccesses} times! Well done!`);
    }
  }, [successCount, requiredSuccesses, targetNote, usePolishNotation]);

  useEffect(() => {
    document.title = "Guitar Note Trainer";
    ...
    <div className="bg-gray-900 text-white min-h-screen flex flex-col items-center pt-8 sm:pt-12 px-4 font-sans">
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-6 text-center">
          ...
          <p className="text-7xl font-bold text-blue-400 mb-4">{getDisplayNote(targetNote, usePolishNotation)}</p>
          ...
          <div className="flex justify-center items-center space-x-4 mb-4 text-sm">
            ...
            <div className="flex items-center space-x-2 text-gray-300">
                <label htmlFor="playSound">Play Note</label>
                <button
                    id="playSound"
                ...
                >
                    <span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform duration-300 ${playNoteSound ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
            </div>
             <div className="flex items-center space-x-2 text-gray-300">
                <label htmlFor="polishNotation">Polish (H/B)</label>
                <button
                    id="polishNotation"
                    onClick={() => setUsePolishNotation(!usePolishNotation)}
                    className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-300 focus:outline-none ${usePolishNotation ? 'bg-blue-500' : 'bg-gray-600'}`}
                >
                    <span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform duration-300 ${usePolishNotation ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
            </div>
          </div>
        ...
        {showDiagram && <FretboardDiagram targetNote={targetNote} usePolishNotation={usePolishNotation} />}
      </div>
    </div>
  );
};
